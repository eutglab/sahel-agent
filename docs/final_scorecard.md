...
| **Reliability** | 9 | 3-level fallback; every tool has a terminal provider; malformed output / provider crash / invalid tool call all handled; 61 tests, `pytest -m offline` green | — | Regression tests added for each failure mode |
